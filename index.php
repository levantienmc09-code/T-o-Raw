...
if(isset($_POST['action']) && $_POST['action']==='login'){
    ...
    if(isset($users[$u]) && password_verify($p,$users[$u]['pass'])){
        ...
        $users[$u]['token'] = $token;
        file_put_contents($USERS_FILE,json_encode($users));
        setcookie('remember_user',$u,time()+60*60*24*30,'/'); // 30 ngày
...
// --- Nếu chưa login, hiển thị nút login/register ---
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['file']) && isset($_POST['code'])){
    ...
    file_put_contents("$user_dir/$f", $_POST['code']);
    header("Location: ".$_SERVER['PHP_SELF']."?edit=$f");
    exit;
}
...
if (isset($_GET['raw'])){
    ...
    $p = "$user_dir/$f";
    if(!file_exists($p)){ http_response_code(404); echo "Không tìm thấy file"; exit; }
...
$edit = isset($_GET['edit']) ? preg_replace('/[^a-zA-Z0-9_\-\.]/','',$_GET['edit']) : '';
$edit_content = $edit && file_exists("$user_dir/$edit") ? file_get_contents("$user_dir/$edit") : '';
?>
<!DOCTYPE html>
<html lang="vi">
<head>
...
<tr>
<td><?=$f?></td>
<td><?php if(filesize("$user_dir/$f")>0){ ?><a href="?raw=<?=urlencode($f)?>" target="_blank">RAW</a><?php } ?></td>
